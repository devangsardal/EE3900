...
\begin{align}
F(z) &= \sum _{n= 0}^{\infty}a^{n}z^{-n}
\\
F(z) &= \sum _{n= 0}^{\infty}(az^{-1})^{n}
\\
...
h(n) = \frac{1}{2\pi}\sum {k=0}^{\infty }h(k)\int{0}^{2\pi}z^{\j(n-k) \omega}d \omega
\end{align}
...
\begin{align}
    \left|h(n)\right| &= \frac{1}{2\pi}\int_{-\pi}^{\pi}H(e^{\j\omega})d\omega
    \\
    &= \frac{1}{2\pi}\int_{-\pi}^{\pi}\frac{4|\cos{\omega}|}{\sqrt{5 + 4\cos{\omega}}}d\omega
\end{align}
\end{enumerate}
